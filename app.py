...
inflation_pct = st.sidebar.number_input(
    ...
    value=3.0, min_value=0.0, max_value=100.0, step=0.5, format="%.1f"
)
...
total_required_today = (monthly_expenses * 12) / gross_return_rate
future_required = total_required_today * ((1 + inflation_rate) ** years_to_retire)
...
col1.metric("Total Required Today (RM)", f"{total_required_today:,.2f}")
col2.metric("Future Required at Retirement (RM)", f"{future_required:,.2f}")
